...
public interface Recover {
     ...
     */
    @Contract(pure = true)
    static <S, F> @NotNull Function<Optional<F>, Result<S, F>> whenAbsent(Supplier<S> onEmpty) {
     ...
     */
    static <S, F, TF extends F> @NotNull Function<F, Result<S, F>> ifType(Class<TF> targetClass, Function<TF, S> mapper) {
     ...
     */
    @Contract(pure = true)
    static <S, F> @NotNull Function<F, Result<S, F>> ifIs(Predicate<F> test, Function<F, S> mapper) {
     ...
     * the input value as a Failure.
     */
    static <S, F> @NotNull Function<F, Result<S, F>> ifNot(@NotNull Predicate<F> test, Function<F, S> mapper) {
     ...
     */
    @Contract(pure = true)
    static <S, F> @NotNull Function<F, Result<S, F>> ifEquals(F expectedValue, Function<F, S> mapper) {
     ...
     */
    @Contract(pure = true)
    static <S, F> @NotNull Function<F, Result<S, F>> ifPresent(Function<F, Optional<S>> successProvider) {
